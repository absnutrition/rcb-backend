...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    res.json({
      summary: { totalOrders: orders.length, totalRevenue: revenue, totalCustomers: parseInt(custsR.rows[0].count), avgOrderValue: paid.length ? revenue/paid.length : 0, pendingPayment: statusBreakdown['pending_payment']||0, inProduction: (statusBreakdown['order_approved']||0)+(statusBreakdown['printed']||0), readyToShip: statusBreakdown['printed']||0 },
